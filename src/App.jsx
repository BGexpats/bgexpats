...
function CategoryPage({catId,setView,lang,t,cache,setCache,user,reviews,setReviews}){
  ...
  const formatBody=(text)=>{
    ...
    return lines.map((line,i)=>{
      if(line.startsWith("## "))
        return<div key={i} className="article-serif" style={{fontSize:19,fontWeight:600,color:C.text,margin:"22px 0 8px",paddingTop:i===0?0:14,borderTop:i===0?"none":`1px solid ${C.border}`}}>{line.slice(3)}</div>
      if(line.startsWith("### "))
        return<div key={i} className="article-serif" style={{fontSize:16,fontWeight:600,color:C.primary,margin:"16px 0 4px"}}>{line.slice(4)}</div>
      if(line.trim()==="---")
      ...
      if(line.startsWith("|")&&line.endsWith("|")){
        ...
        return<div key={i} style={{display:"grid",gridTemplateColumns:`repeat(${cells.length},1fr)`,gap:8,padding:"7px 0",borderBottom:`1px solid ${C.border}`,fontWeight:isHeader?700:400,fontSize:13.5,color:C.text}}>
          {cells.map((c,ci)=><div key={ci}>{renderInline(c)}</div>)}
        </div>
      }
      if(line.startsWith("• ")||line.startsWith("- "))
        return<div key={i} className="article-serif" style={{display:"flex",gap:8,margin:"4px 0",fontSize:14.5,lineHeight:1.7,color:C.text}}><span style={{color:C.accent,flexShrink:0}}>•</span><span>{renderInline(line.slice(2))}</span></div>
      if(line.match(/^\d+\./))
        return<div key={i} style={{margin:"4px 0",paddingLeft:4,fontSize:14.5,lineHeight:1.7,color:C.text}}>{renderInline(line)}</div>
      if(line.startsWith("💡"))
        return<div key={i} style={{background:"#fdf6ec",border:"1px solid #ecd9b4",borderRadius:10,padding:"11px 14px",margin:"14px 0 0",fontSize:13.5,lineHeight:1.6,color:"#7a5320"}}>{renderInline(line)}</div>
      const boldLead=line.match(/^\*\*(.+?)\*\*\s*(.*)$/)
      if(boldLead){
        const[,label,rest]=boldLead
        if(!rest)return<div key={i} className="article-serif" style={{fontSize:16,fontWeight:600,color:C.text,margin:"14px 0 4px",lineHeight:1.5}}>{label}</div>
        return<div key={i} style={{margin:"10px 0 2px",fontSize:14.5,lineHeight:1.7,color:C.text}}><span style={{fontWeight:700,color:C.primary}}>{label}</span> {renderInline(rest)}</div>
      }
      if(line.trim()==="")return<div key={i} style={{height:6}}/>
      return<div key={i} style={{lineHeight:1.75,color:C.text,fontSize:14.5}}>{renderInline(line)}</div>
    })
  }
    ...
    <div style={{minHeight:"100vh",background:C.page}}>
      <div style={{maxWidth:820,margin:"0 auto",padding:"32px 20px 60px"}}>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:26}}>
          ...
          <div>
            <h1 className="article-serif" style={{fontSize:24,fontWeight:600,color:C.text,margin:0}}>{lb.label}</h1>
            <p style={{fontSize:13,color:C.muted,margin:"3px 0 0"}}>{lb.sub}</p>
          </div>
        </div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",margin:"0 0 14px"}}>
          <span style={{fontSize:12.5,fontWeight:600,color:C.muted,textTransform:"uppercase",letterSpacing:"0.04em"}}>{t.inThisTopic||"In this topic"}</span>
          <span style={{fontSize:13,color:C.muted}}>{cat.articles.length} {cat.articles.length>1?t.guides:t.guide}</span>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {cat.articles.map((art,i)=>{
            ...
            return(
              <div key={i} style={{background:C.surface,border:`1px solid ${isOpen?C.primary:C.border}`,borderRadius:14,overflow:"hidden",boxShadow:"0 1px 3px rgba(0,0,0,0.04)",transition:"border-color 0.2s"}}>
                <button onClick={()=>handleOpen(i)} style={{width:"100%",background:"none",border:"none",padding:"16px 18px",cursor:"pointer",textAlign:"left",display:"flex",justifyContent:"space-between",alignItems:"center",gap:12}}>
                  <div style={{display:"flex",alignItems:"center",gap:12,minWidth:0}}>
                    <span style={{flexShrink:0,width:26,height:26,borderRadius:"50%",background:cat.bg,color:C.primary,fontSize:12,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center"}}>{i+1}</span>
                    <div style={{minWidth:0}}>
                      <div style={{fontWeight:600,fontSize:14.5,color:C.text}}>{art.titles[lang]||art.titles.en||art.titles.en}</div>
                      {art.readTime&&<div style={{fontSize:12,color:C.muted,marginTop:2}}>{art.readTime}</div>}
                    </div>
                  </div>
                  <span style={{color:C.muted,fontSize:16,flexShrink:0,transform:isOpen?"rotate(180deg)":"none",transition:"transform 0.2s"}}>⌄</span>
                </button>
                {isOpen&&(
                  <div style={{padding:"0 20px 20px 58px",borderTop:`1px solid ${C.border}`,paddingTop:16}}>
                    {isLoading?(
...
export default function App(){
  ...
  return(
    <div style={{fontFamily:"'Figtree',system-ui,-apple-system,sans-serif",background:C.page,minHeight:"100vh"}}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Bricolage+Grotesque:opsz,wght@12..96,400;12..96,500;12..96,600;12..96,700&family=Figtree:wght@300;400;500;600;700&family=Lora:ital,wght@0,500;0,600;0,700;1,500&display=swap');
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .article-serif{font-family:'Lora',Georgia,serif}
